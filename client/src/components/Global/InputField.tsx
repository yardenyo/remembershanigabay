...
const inputField = ({
  ...
  return (
    <div
      className={`relative ${
        isTextarea ? "h-auto" : "h-14"
      } w-full min-w-[200px] mb-4`}
    >
      {isTextarea ? (
        <textarea
          id={id}
          ...
          value={value}
          onChange={onChange}
          className="peer h-32 w-full border-b border-blue-gray-200 bg-transparent pt-2 pb-1 font-sans text-sm font-normal text-blue-gray-700 outline outline-0 transition-all placeholder-shown:border-blue-gray-200 focus:border-black focus:outline-0 disabled:border-0 disabled:bg-blue-gray-50 resize-none"
          placeholder=" "
        />
      ) : (
        <input
